...
- The API vs local LLM decision comes down to three variables: scale (API wins below ~50K requests/day), privacy requirements (local wins when data cannot leave your environment), and capability needs (frontier models still lead on complex reasoning).
- GPU compute is not the only cost of self-hosting — ML engineer time, monitoring, redundancy, and model updates add 50–100% to the raw GPU compute cost; always calculate full TCO.
- Ollama provides an OpenAI-compatible API endpoint at `localhost:11434/v1`, which means existing OpenAI SDK code switches to local inference by changing only the base URL.
- At 1,000 requests/day, GPT-4o-mini API costs approximately $22.50/month; a single A10G GPU instance costs $2,520/month regardless of utilization — self-hosting is uneconomical at low traffic.
- At 500,000 requests/day, self-hosting capable open-source models can cost 60–80% less than GPT-4o-mini API at the same quality level.
- 4-bit quantized models (GGUF format via llama.cpp or Ollama) run on consumer hardware at a fraction of full-precision cost with minimal quality degradation for most tasks.
- A hybrid approach — API for complex user-facing queries, local model for high-frequency internal classification and extraction — captures most cost savings while maintaining quality.
- Benchmark on your specific tasks, not overall model benchmarks — MMLU scores do not predict performance on your actual use case.

---

## FAQ

**Can Llama 3.1 or Mistral replace GPT-4o for production use?**
...
**What hardware do I need to run a 70B parameter model?**

At 4-bit quantization (GGUF format via llama.cpp or Ollama): ~40GB VRAM — achievable on 2x RTX 4090 (24GB each) or 1x A100 80GB. At full f16 precision: ~140GB VRAM — requires multiple A100 or H100 GPUs. For most teams, 4-bit quantized 70B on cloud GPU instances is the practical starting point.

**Is running LLMs locally truly private?**
...
**What is the minimum request volume where self-hosting makes financial sense?**
...
**What are the regulatory requirements that mandate local deployment?**

HIPAA (US healthcare), GDPR (EU personal data with strict data residency requirements), FedRAMP (US federal government data), and SOC 2 Type II audits that prohibit sending customer data to third-party AI services all create mandates for local or private cloud deployment. Review your compliance requirements before selecting an architecture.

**How do I evaluate whether an open-source model is good enough for my task?**

Collect 100 representative inputs from your application with known correct outputs or human-evaluated quality scores. Run them through both the API model and your candidate open-source model. Compute quality metrics (accuracy, BLEU, or human preference rate) for each. If the quality gap is below your acceptable threshold and the cost savings are significant, switch.
...
- [Run LLMs Locally with Ollama](/blog/run-llms-locally/)
- [Open Source LLM Guide](/blog/open-source-llm-guide/)
