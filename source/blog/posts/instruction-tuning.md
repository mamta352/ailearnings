...
level: "intermediate"
time: "18 min"
stack: ["Python", "HuggingFace", "TRL", "Unsloth"]
---
...
- Instruction tuning is the process that converts a raw base model (a next-token predictor) into a helpful assistant by training on instruction-response pairs with supervised fine-tuning.
- Loss is computed only on assistant response tokens, not on instruction or system tokens — `SFTTrainer` handles this automatically when using the messages format; do not implement this manually.
- Always apply the model's chat template using `tokenizer.apply_chat_template()` — never build the template string manually, as each model family uses different special tokens and formatting rules.
- Set `add_generation_prompt=False` during training (the full response is present) and `add_generation_prompt=True` during inference (the model needs to generate the response).
- Vary instruction types deliberately: include explain, summarize, generate, classify, compare, debug, and critique examples — a model trained only on "explain X" will struggle with "write a plan for Y" even for in-domain topics.
- Multi-turn conversation datasets are more powerful than single-turn examples because they teach the model to maintain context across a conversation; all assistant turns contribute to the loss.
- Including a small set of examples showing explicit refusals for critical safety constraints is more reliable than relying on the base model's alignment surviving fine-tuning.
- Instruction tuning is the foundation that RLHF and DPO build on — you need a well-instruction-tuned SFT model before preference-based alignment can improve it meaningfully.
...
**How specific should my system prompt be in training examples?**
More specific is better for domain tasks. A generic "You are a helpful assistant" system prompt works for general fine-tuning but leaves performance on the table. A domain-specific system prompt — "You are a customer support agent for a SaaS platform" — provides consistent persona context and reduces the behavioral learning the data needs to accomplish. Keep the training system prompt identical to what you will use at inference.

**How do I verify my chat template is applied correctly before training?**
Print one formatted training example and manually verify that the system, user, and assistant sections contain the expected special tokens. For Llama 3, look for `<|begin_of_text|>`, `<|start_header_id|>system<|end_header_id|>` markers. For Mistral, verify the `[INST]` and `[/INST]` delimiters. Any deviation from the expected format means the model is training on malformed data.

**What instruction types should I include in my dataset?**
Plan for at least six instruction types: explain (conceptual understanding), generate (create new content), summarize (condense information), classify (categorize input), compare (evaluate two things), and debug or critique (identify and fix problems). Uniform coverage ensures the fine-tuned model handles the full range of requests in your domain, not just the most common one.
