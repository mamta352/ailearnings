...
Use LoRA when you are adapting model behavior or style, working with limited compute, or iterating quickly. Use full fine-tuning when you need the absolute highest quality, when LoRA quality tests consistently fall short after rank tuning, or when you are making major domain shifts with thousands of examples and have the infrastructure to run it.

The default should be LoRA. The threshold for switching to full fine-tuning is a documented, measured quality gap on your specific task that justifies the infrastructure cost.
...
The rule of thumb: full fine-tuning requires roughly 14-20 bytes per parameter. For a 7B model (7 × 10^9 parameters), that is 98–140 GB VRAM. At bf16 (2 bytes/param) for weights + gradients + 8 bytes for Adam optimizer states: 2 + 2 + 8 = 12 bytes/param = 84 GB minimum. In practice, activations push this to ~100 GB.
...
<script type="application/ld+json">
{
  ...
  "mainEntity": [
    {
      "@type": "Question",
      "name": "Can LoRA match full fine-tuning quality?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "For task adaptation — output format, style, narrow tasks — yes, LoRA at r=32 or r=64 consistently matches full fine-tuning. For major domain shifts requiring deep knowledge restructuring, full fine-tuning has an edge, though the gap is often smaller than expected."
      }
    },
    {
      "@type": "Question",
      "name": "How much VRAM does full fine-tuning really need?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Full fine-tuning requires roughly 14-20 bytes per parameter. For a 7B model: weights + gradients + Adam states = ~12 bytes/param = 84 GB minimum. Activations push this to ~100 GB, requiring at least 2×A100 80GB GPUs."
      }
    },
    {
      "@type": "Question",
      "name": "Is there a middle ground between LoRA and full fine-tuning?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Yes — increasing LoRA rank and adding more target modules gradually approaches full fine-tuning capacity. LoRA at r=64–128 across all layers covers the middle ground. There is also GaLore (Gradient Low-Rank Projection) which enables full fine-tuning at LoRA-like memory."
      }
    },
    {
      "@type": "Question",
      "name": "Which is better for deployment?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "LoRA adapters are lighter (50–500MB vs 15GB) and can be swapped at runtime. For production at scale, the merged LoRA model is often the best compromise — small adapter training, standard deployment without adapter overhead."
      }
    }
  ]
}
