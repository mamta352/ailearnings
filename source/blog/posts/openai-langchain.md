...
- LangChain adds value for multi-step pipelines (RAG, agents, memory management) but is over-engineered for simple single-step completions — use the raw OpenAI SDK for those.
- LCEL (LangChain Expression Language) uses the `|` pipe operator to compose Runnables — every component (prompt, model, parser, retriever) implements `.invoke()`, `.stream()`, `.batch()` uniformly.
- Avoid legacy chain classes (`LLMChain`, `SequentialChain`, `ConversationalRetrievalChain`) — they do not support streaming natively and are harder to debug; LCEL is the recommended pattern.
- `ChatPromptTemplate` with `MessagesPlaceholder` is the clean way to inject conversation history into prompts — it keeps history separate from static instructions.
- `RunnableWithMessageHistory` with a `session_id` key provides session-isolated conversation memory — without session isolation, all users share the same history object.
- Store conversation history in Redis or a database for production — in-memory `ChatMessageHistory` does not persist across server restarts.
- Enable LangSmith tracing (`LANGCHAIN_TRACING_V2=true`) in production — it traces every step, shows token usage per component, and makes debugging chain failures tractable.
- LangChain works with Anthropic and Gemini via drop-in swaps: `ChatAnthropic` or `ChatGoogleGenerativeAI` replace `ChatOpenAI` while the rest of the LCEL chain stays identical.
...
Wrap any function in `RunnableLambda` to make it LCEL-compatible: `RunnableLambda(my_function)`. This lets you mix raw API calls with LangChain components in the same chain. Useful for performance-critical steps where you want direct SDK access.

**How do I debug a LangChain chain that is returning wrong answers?**

Enable LangSmith tracing to see exactly what prompt was sent to the model, what the model returned at each step, and where the chain diverged from expectations. Without tracing, you are guessing which step produced the wrong output. LangSmith replays are especially valuable for RAG failures where the retrieved context is wrong or incomplete.

**What chunk size should I use for document splitting?**

Start with `chunk_size=1000` and `chunk_overlap=200` as defaults, then tune based on your document structure. Technical documentation with short sections works better with smaller chunks (500–700 tokens). Long-form narrative text works better with larger chunks (1000–1500 tokens). Always evaluate retrieval quality on a representative query set before choosing final parameters.

**When should I use RunnableParallel vs sequential chain steps?**

Use `RunnableParallel` when two or more steps do not depend on each other's output and can run simultaneously — for example, retrieving context from multiple sources while also counting tokens. Use sequential composition (just pipe `|`) when each step depends on the previous output. Parallel execution reduces total latency for independent steps.
...
- [Build a RAG App with LangChain](/blog/build-rag-app/)
- [LangChain RAG Tutorial](/blog/langchain-rag-tutorial/)
