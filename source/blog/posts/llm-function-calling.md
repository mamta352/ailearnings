...
Function calling is the mechanism that turns an LLM from a text generator into an agent that can interact with the real world. The pattern is: declare tools with JSON schemas, run the tool execution loop, send results back as tool messages, and repeat until the model produces a final text response. Parallel tool calls reduce latency for multi-source queries. Tool choice control lets you force or prevent tool usage. The same pattern extends to any external system — databases, APIs, code execution engines — making it the foundation of every AI agent architecture.
