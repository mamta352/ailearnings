...
async function main() {
  ...
  if (ranked) {
    // --ranked: submit only pages that have GSC impressions > 0 (from seo-master.json)
    ...
    urls = pages.filter(p => p.impressions > 0).map(p => p.url);
    console.log(`  🎯 Submitting ${urls.length} ranked pages (impressions > 0)`);
  } else if (urlsArg) {
